...
export default function ChatRoom() {
    ...
    const {sendMessage, chatData, connectionData, stompClient, setErrorMessage, setShowNotification, setNotificationMessage, setIsSuccessNotification} = useContext(ChatContext);
                ...
                isChatEnded &&
                    ...
                    <div className="px-8 md:px-24 pt-32 md:pt-40 pb-8 md:pb-24 w-full h-full">
                        <div className="w-full h-full flex flex-col p-4 bg-gray-300 bg-opacity-95 rounded shadow-xl relative">
                            <DotsHorizontalIcon onClick={() => setIsMenuVisible(!isMenuVisible)} alt="chat_menu" className="absolute w-7 cursor-pointer right-7 top-0 text-gray-600" />
                            <div className={`absolute right-7 top-6 bg-opacity-60 rounded-lg bg-gray-400 flex flex-col gap-y-1 p-1 transition duration-300 opacity-0 ${isMenuVisible ? "opacity-100" : "opacity-0" } `}>
                                <div
                                    onClick={() => {reportUser()}}
